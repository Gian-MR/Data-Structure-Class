...
public class SLToBSTWrapper {
    ...
    public static class BinarySearchTree<K, V> implements Map<K, V> {
        ...
        public V put(K key, V value) {
            if (this.root == null) {
                ...
                this.root = new BinaryTreeNodeImp<MapEntry<K, V>>(
                        M, null, null, null);
                this.currentSize++;
                return value;
            } else {
        ...
        private V putAux(K key, V value, BinaryTreeNodeImp<MapEntry<K, V>> N) {
            ...
            if (comparison < 0) {
                ...
                if (N.getRightChild() == null) {
                    MapEntry<K, V> M = new MapEntry<K, V>(key, value);
                    BinaryTreeNodeImp<MapEntry<K, V>> newNode = new BinaryTreeNodeImp<MapEntry<K, V>>(
                            M, null, null, N);
        ...
        public boolean isBalanced() {
            if (this.isEmpty() || this.size() == 1) {
                ...
                return Math
                        .abs(this.heightAux(this.root.getLeftChild()) - this.heightAux(this.root.getRightChild())) <= 1;
            }
        }

        private int heightAux(BinaryTreeNode<MapEntry<K, V>> N) {
            if (N == null) {
                return 0;
            } else if ((N.getLeftChild() == null) && (N.getRightChild() == null)) {
                return 0;
            } else {
                return 1 + Math.max(this.heightAux(N.getLeftChild()), this.heightAux(N.getRightChild()));
            }
        }
    ...
    /*
     * Write a non-member method named sortedListToBST().
     * 
     * The method receives a non-empty singly linked list of integers sorted in
     * ascending (increasing) order, and it should * convert that list into a
     * height-balanced Binary Search Tree (BST).
     * 
     * Note: The tester will use BST method isBalanced() (see given code) to check
     * if the tree is balanced.
     * 
     * Hints:
     * 1. Recall binary search on an array of integers.
     * 2. Use a divide-and-conquer strategy
     * 3. Write an auxiliary function that moves over the singly-linked list as if
     * doing a binary search: split the list into * halves and always put in the BST
     * the element in the midpoint of the list segment being processed.
     */
