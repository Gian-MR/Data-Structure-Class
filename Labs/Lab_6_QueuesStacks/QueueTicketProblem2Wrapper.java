...
public class QueueTicketProblem2Wrapper {
   ...
   // @SuppressWarnings({ "unchecked", "rawtypes" })
    public static int satisfiedCustomers(int[] ticketNeeds, int availableTickets) {
        // Add code here
        Queue<Integer> line = new DoublyLinkedQueue<>();
        int satisfied = 0;

        for (int need : ticketNeeds) {
            line.enqueue(need);
        }

        while(availableTickets > 0 && !line.isEmpty()){
            int need = line.dequeue();
            need--;
            availableTickets--;
            if(need == 0){
                satisfied++;
